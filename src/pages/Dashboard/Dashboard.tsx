            ...
            <nav className={openMenu?navMenu : navMenuClosed}>
                ...
                <h1 className={openMenu?navTitle: navTitle + ' opacity-0 '}>Navigation menu Taxi</h1>
                    <NavLink to='/' className={({ isActive }) => isActive ?( openMenu ? navItemActive:navItemActiveClosed ):( openMenu? navItem: navItemClosed)}>
                        { ({isActive})=>(<><div className={isActive ? activeIcon:icon}><CiCalendar /></div><span className={openMenu? 'duration-[2000ms]':' opacity-0 duration-200'}>{isFrench? 'Calendrier' : 'Calendar'}</span></>) }
                    </NavLink>
...
const navMenu = 'px-4 py-10 flex flex-col fixed top-0 left-0 z-50 bg-white bg-opacity-75  ease-linear duration-500 w-[200px] min-h-screen h-full shadow-lg '

const navMenuClosed = 'px-1 py-10 flex flex-col fixed top-0 left-0 z-50 bg-white bg-opacity-75  ease-linear duration-500 w-[55px] min-h-screen h-full shadow-lg '
const menuIcon = 'absolute top-4 text-2xl right-2 cursor-pointer hover:text-purple-500'
const screen =  'relative flex justify-center min-w-screen w-full bg-gray-50 min-h-screen '